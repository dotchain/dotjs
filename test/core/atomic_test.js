...
describe("Atomic - interop serialization", () => {
    it("should serialize", () => {
        ...
        expect(JSON.stringify(new Atomic(new Null))).to
            .equal('[{"changes.empty":[]}]')
    })
    
    it("should deserialize", () => {
        const d = new FakeDecoder;
        expect(Atomic.fromJSON(d, [null])).to
            .deep.equal(new Atomic)
        expect(Atomic.fromJSON(d, [{"changes.empty":[]}])).to
            .deep.equal(new Atomic(new Null))
    });
});
